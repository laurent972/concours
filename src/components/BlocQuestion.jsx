        ...
        <form id={question} onSubmit={handleSubmit}>
          {
              array.map((key,id) => (
                <div key={id} className='flex gap-2'>
                  {goodAnswer.length > 1 ? 
                    <input id={key[0] + key[1]}  className="hidden peer" type="checkbox" name='absw' value={key[0]} onChange={e => setAnswer(...answer.push(e.target.value))} /> 
                     :
                     <input id={key[0] + key[1]}  className="hidden peer" type="radio" name='reponse' value={key[0]} onChange={e => setAnswer(e.target.value)} /> 
                  } 
